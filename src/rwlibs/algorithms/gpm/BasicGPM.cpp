...
Q BasicGPM::solve(const Q& q, const Q& dq, const VelocityScrew6D<>& tcpscrew) {
    RW_ASSERT(q.size() == _dof);
    RW_ASSERT(dq.size() == _dof);
...
vector<double> BasicGPM::getCostGradient(const Q& q, const matrix<double>& jac) {
    ...
    if (_useSingularityCost) {
        ...
        for (int i = 0; i<_dof; i++) {
            ...
            double ddet = (LinearAlgebra::Det(jtj2) - LinearAlgebra::Det(jtj1)) / (2*delta);
...
void BasicGPM::calculatePosAndVelLimits(vector<double>& lower,
                                        vector<double>& upper,
                                        const Q& q,
                                        const Q& dq)
{
