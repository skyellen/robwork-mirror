...
/**
 * @brief Solve the inverse kinematics problem with respect to joint limits and collisions-
 * 
 * Given an arbitrary iterative inverse kinematics solver, the IKMetaSolver attempts to find 
 * a collision free solution satisfying joint limits. It repeatingly calls the iterative solver
 * with new random start configurations until either a solution is found or a specified max attempts
 * has been reached.
 */
...
public:
     ...
     * 
     * The IKMetaSolver takes ownership of the \b iksolver.
     * The IKMetaSolver does NOT take ownership of the \b collisionDetector. To skip 
     * testing for collision use null as collision detector 
     * 
     * @param iksolver [in] The basic iksolver to use. Ownership is taken
     * @param device [in] Device to solve for
     * @param collisionDetector [in] CollisionDetector to use. If null no collision detection used.
     */
	IKMetaSolver(IterativeIK* iksolver,
	 ...
	 * 
	 * Tries to solve the inverse kinematics problem using the iterative inverse kinematics
	 * solver provided in the constructor. It tries at most \b cnt times and can either
	 * be stopped after the first solution is found or continue to search for solutions. If multiple
	 * solutions are returned there might be dublicates in the list.
	 * 
	 ...
	 * @param stopatfirst [in] If true the method will return after the first solution is found. 
	 * If false it will continue searching for more solution until the maximal number of attemps is met.
	 */
	std::vector<rw::math::Q> solve(const math::Transform3D<>& baseTend, 
...
#endif //#ifndef RW_INVERSEKINEMATIOCS_IKMETASOLVER_HPP
