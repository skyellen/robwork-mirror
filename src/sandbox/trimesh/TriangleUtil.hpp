#ifndef TRIANGLEUTIL_HPP_
#define TRIANGLEUTIL_HPP_
													 ...
													 double epsilon=0.0000001)
	{
		...
		for(size_t i = 0; i<triMesh.getSize(); i++){
			...
			for(size_t j=0; j<vertices->size(); j++){
				...
				if( MetricUtil::Norm2Dist<T>(v, tri.getVertex(0))<epsilon )
					v0 = j;
				if( MetricUtil::Norm2Dist<T>(v, tri.getVertex(1))<epsilon )
					v1 = j;
				if( MetricUtil::Norm2Dist<T>(v, tri.getVertex(2))<epsilon )
					v2 = j;				
			}
			if( v0<0 ){
				v0 = vertices->size();
