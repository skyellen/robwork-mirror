...
void XMLTaskSaver::writeEntityInfo(EntityPtr entity, DOMElement* element, DOMDocument* doc) {
	...
	XMLPropertySaver::save(entity->getPropertyMap(), element, doc);
...
bool XMLTaskSaver::save(rw::task3::QTaskPtr task, const std::string& filename) {
	XMLTaskSaver saver;
	return saver.saveImpl<Q>(task, filename);
}


bool XMLTaskSaver::save(rw::task3::CartesianTaskPtr task, const std::string& filename) {
	XMLTaskSaver saver;
