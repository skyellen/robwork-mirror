	...
	for(XMLSize_t i = 0; i < nodeCount; ++i ) {
		...
		if (child != NULL) {
			...
			if (XMLString::equals(XMLTaskFormat::EntityIdId, child->getNodeName())) {
				try {
					entity->setId(XMLBasisTypes::readString(child, false));
				} catch (...) {}
			}
...
void XMLTaskLoader::readEntities(DOMElement* element, Ptr<Task<T> > task) {
	...
	for(XMLSize_t i = 0; i < nodeCount; ++i ) {
		...
		if (child != NULL) {
			...
			} else if (XMLString::equals(XMLTaskFormat::QTargetId, child->getNodeName())) {
...
rw::task3::QTaskPtr XMLTaskLoader::getQTask() {
	if (_qTask == NULL)
		RW_THROW("No QTask Loaded");
	return _qTask;
}

rw::task3::CartesianTaskPtr XMLTaskLoader::getCartesianTask() {
	if (_cartTask == NULL)
		RW_THROW("No CartesianTask Loaded");
	return _cartTask;
}

rw::task3::TaskBasePtr XMLTaskLoader::getTask() {
	if (_task == NULL)
