...
public:
   ...
   /**
	...
	/**
	 * @brief Total length of the trajectory.
	 * 
	 * The length of the Trajectory corresponds to the time it takes to run through it.
	 */
	 ...
	 */
	void add(rw::sandbox::Blend<T>* blend,
	         rw::sandbox::Interpolator<T>* interpolator) {
        Segment segment;
	...
	void add(Trajectory<T>* trajectory) {
	    BOOST_FOREACH(const Segment& segment, trajectory->_segments) {
	       ...
	       newSegment.t2 = newSegment.t1 + segment.interpolator->getLength();
