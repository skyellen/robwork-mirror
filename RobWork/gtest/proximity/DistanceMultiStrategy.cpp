...
TEST_P(DistanceMultiStrategyTest, Plane_Triangle) {
	...
	ASSERT_EQ(1,res.p1s.size());
	ASSERT_EQ(1,res.p2s.size());
	ASSERT_EQ(1,res.p1prims.size());
	ASSERT_EQ(1,res.p2prims.size());
	ASSERT_EQ(1,res.distances.size());
...
TEST_P(DistanceMultiStrategyTest, Plane_Cuboid) {
	...
	EXPECT_EQ(modelA,res.a);
	EXPECT_EQ(modelB,res.b);
	ASSERT_EQ(11,res.p1s.size());
	ASSERT_EQ(11,res.p2s.size());
	ASSERT_EQ(11,res.p1prims.size());
	ASSERT_EQ(11,res.p2prims.size());
	ASSERT_EQ(11,res.distances.size());
	EXPECT_EQ(res.p1,res.p1s[0]);
	EXPECT_EQ(res.p2,res.p2s[0]);
	EXPECT_EQ(res.p1[0],res.p2[0]);
	EXPECT_EQ(res.p1[1],res.p2[1]);
