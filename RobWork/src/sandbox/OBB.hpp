...
public:
	...
	OBB(rw::math::Transform3D<T> t3d,
		rw::math::Vector3D<T> halfLng):
		_t3d(t3d),
		_halfLng(halfLng)
	{}
	...
	//}
	 ...
	 static OBB<T> buildTightOBB1(const rw::geometry::TriMesh& tris, size_t index = 0){
...
#ifdef skjdksdfj
	/**
	 * @brief builds a tight fitting OBB from an indexed vertice array.
	 * Only vertices pointed to by the index array are used to construct
	 * the OBB.
	 */
	template <class VERTEXARRAY>
	static OBB<T> BuildTightOBB(const VERTEXARRAY& verts, size_t index = 0){
		using namespace boost::numeric;
		using namespace rw::math;
		using namespace rw::geometry;

		// 1. Compute convex hull
		//std::cout << "build hull from: " << verts.size() <<  std::endl;
		//PlainTriMesh< TriangleN1<T> > *triHull =
		//	IncrementalHull<T>::template build<VERTEXARRAY>(verts);
		int nrOfTris = verts.size();
		//if( triHull )
		//    nrOfTris = triHull->getSize();
		//std::cout << "TriHull size: " << nrOfTris << std::endl;
		//std::ostringstream ostr;
		//ostr << "testHull" << index << ".stl";
		//std::string str = ostr.str();
		//STLFile::WriteSTL<T>(*(((TriMesh<T>*)&triHull)), str );
		//std::cout << "Writing to file: " << ostr.str() << std::endl;

		// 2. Compute centroid for convex hull
		// 2.1 centroid is computed using the triangles of the convex hull
		//ublas::bounded_matrix<T,3,3> covar;
		ublas::matrix<T> covar( ublas::zero_matrix<double>(3, 3) );
		Vector3D<T> centroid(0,0,0);

        // ok so a bit bad but we only the vertices directly
        for(size_t i = 0; i<verts.size(); i++ ){
            //std::cout << "i" << i << std::endl;
            // calc triangle centroid
            Vector3D<T> c = verts[i];
		...
		centroid = centroid/verts.size();
		// 3. Compute Covariance matrix
		// 3.1 using the variables from 2.1 we create the covariance matrix
		for(size_t j=0;j<3;j++)
			for(size_t k=0;k<3;k++)
				covar(j,k) = covar(j,k)-centroid[j]*centroid[k];
		...
		Vector3D<T> p = rotInv * verts[0];
		Vector3D<T> max=p, min=p;
		for(size_t i = 1; i<verts.size(); i++ ){
			Vector3D<T> p = rotInv * verts[i];
			for(int j=0; j<3; j++){
                if( p(j)>max(j) ) max(j) = p(j);
                else if( p(j)<min(j) ) min(j) = p(j);
			}
