...
#ifndef RW_GEOMETRY_OBRSS_HPP_
#define RW_GEOMETRY_OBRSS_HPP_
...
/**
 * @brief class representing an Oriented Bounding Box (OBRSS)
 */
template <class T=double>
class OBRSS: public OBV<OBRSS<T> >{
private:
...
public:
	...
	OBRSS(rw::math::Transform3D<T> t3d, rw::math::Vector3D<T> halfLng):
		_t3d(t3d),
		_halfLng(halfLng)
	{}
    ...
    /**
     * @brief Ouputs OBRSS to stream
     * @param os [in/out] stream to use
     * @param OBRSS [in] oriented bounding box
     * @return the resulting stream
     */
    friend std::ostream& operator<<(std::ostream& os, const OBRSS<T>& OBRSS){
        return os <<" OBRSS { \n"
        			<< OBRSS._halfLng << "\n\t"
        			<< OBRSS._t3d.P() << "\n\t"
        			<< OBRSS._t3d.R() << "\n}";
	 ...
	 static OBRSS<T> buildTightOBRSS(const rw::geometry::TriMesh& tris, size_t index = 0){
	        ...
	        return OBRSS<T>(trans, halfLength);
	    }
...
//! define traits of the OBRSS
template<typename T> struct Traits<geometry::OBRSS<T> >{ typedef T value_type; };
