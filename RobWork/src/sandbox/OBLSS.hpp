...
#ifndef RW_GEOMETRY_OBLSS_HPP_
#define RW_GEOMETRY_OBLSS_HPP_
...
/**
 * @brief class representing an Oriented Bounding Box (OBLSS)
 */
template <class T=double>
class OBLSS: public OBV<OBLSS<T> >{
private:
...
public:
	...
	OBLSS(rw::math::Transform3D<T> t3d, rw::math::Vector3D<T> halfLng):
		_t3d(t3d),
		_halfLng(halfLng)
	{}
    ...
    /**
     * @brief Ouputs OBLSS to stream
     * @param os [in/out] stream to use
     * @param OBLSS [in] oriented bounding box
     * @return the resulting stream
     */
    friend std::ostream& operator<<(std::ostream& os, const OBLSS<T>& OBLSS){
        return os <<" OBLSS { \n"
        			<< OBLSS._halfLng << "\n\t"
        			<< OBLSS._t3d.P() << "\n\t"
        			<< OBLSS._t3d.R() << "\n}";
	 ...
	 static OBLSS<T> buildTightOBLSS(const rw::geometry::TriMesh& tris, size_t index = 0){
	        ...
	        return OBLSS<T>(trans, halfLength);
	    }
...
//! define traits of the OBLSS
template<typename T> struct Traits<geometry::OBLSS<T> >{ typedef T value_type; };
