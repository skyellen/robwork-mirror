...
#define SIZE_ADDR 0x02
#define OFFSET_ADDR 0x0A
#define DIP_SIZE_ADDR 0x0E
...
rw::sensor::ImagePtr ImageLoader::load(const std::string& filename)
{
    CBitmap bmp;
    bool loadsuccess = bmp.Load( filename.c_str() );

    if( !loadsucess ){
        RW_THROW("Error parsing file: "<< filename);
    }
    // convert it to a robwork Image file
