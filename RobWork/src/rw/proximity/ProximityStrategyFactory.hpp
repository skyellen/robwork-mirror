...
#ifndef RW_PROXIMITY_ProximityStrategyFactory_HPP
#define RW_PROXIMITY_ProximityStrategyFactory_HPP
...
namespace rw { namespace proximity {
    ...
    public:
        ...
        typedef boost::function<CollisionStrategy*()> makeCollisionStrategyFunctor;
        static void addCollisionStrategy(const std::string& ID, makeCollisionStrategyFunctor constructor);

        typedef boost::function<DistanceStrategy*()> makeDistanceStrategyFunctor;
        static void addDistanceStrategy(const std::string& ID, makeDistanceStrategyFunctor constructor);
        ...
        static bool addCollisionStrategy(const std::string& name){
            ...
            ProximityStrategyFactory::addCollisionStrategy(name, cstrategy);
            return true;
        }

        template<class T>
        static bool addDistanceStrategy(const std::string& name){
            ProximityStrategyFactory::makeDistanceStrategyFunctor cstrategy =
                    boost::lambda::bind( boost::lambda::new_ptr<T>());

            ProximityStrategyFactory::addDistanceStrategy(name, cstrategy);
            return true;
        }
