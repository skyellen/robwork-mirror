...

{
    ...
    Frame *f1Base = hand.getActiveJoint(3);
    Frame *f2Base = hand.getActiveJoint(5);
        ...
        int psiRes, int phiRes)
{
    ...
    ublas::vector<double> appAngles;// = getApproachAngles();
    std::vector<Contour2DInfoMap::ContactPtrList> contactLists( appAngles.size() );
    for(int i=0; i<appAngles.size(); i++){
        double angle = appAngles[i];

        contactLists[i] = _infoMap.getCNormals(angle, _acceptPerp, _sqrCurvThresScale);
        if( contactLists[i].size()==0 )
            continue;
    ...
    for(int psiIdx=0; psiIdx<psiRes; psiIdx++){
        ...
        for(int phiIdx=0; phiIdx<phiRes; phiIdx++ ){
            ...
            BOOST_FOREACH(Contact *thumCon, thumList){
                if( thumCon==NULL )
                    continue;
                BOOST_FOREACH(Contact *f1Con, f1List){
                    if( f1Con==NULL )
                        continue;
                    ...
                    BOOST_FOREACH(Contact *f2Con, f2List){
                        if( f2Con==NULL )
                            continue;
