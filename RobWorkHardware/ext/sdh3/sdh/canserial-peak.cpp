   ...
   throw (cCANSerial_PEAKException*)
{
    if (pimpl->peak_handle == NULL)
    {
        ...
        dbg << "Opening PEAK CAN baudrate: " << baudrate << ", id_read: 0x" << std::hex << id_read << ", id_write: 0x" << id_write << std::dec << "\n";
        ...
        pimpl->rc = CAN_MsgFilter( USE_HANDLES(pimpl->peak_handle) DWORD(id_write), DWORD(id_read), MSGTYPE_STANDARD);
        if ( pimpl->rc )
        {
#if ! defined( OSNAME_LINUX )
            pimpl->peak_handle = NULL;
    ...
    do
    {
        ...
        if ( bytes_read < size )
        {
            ...
            if ( r_timeout_us == 0 )
                ...
                pimpl->rc = LINUX_CAN_Read_Timeout( pimpl->peak_handle, &(pimpl->m_cmsg),  0 );
            else
