  ...
  \internal
    ...
    \subsection sdhlibrary_cpp_sdh_cpp_details SVN related, detailed file specific information:
      ...
      \par SVN file revision:
        $Id: sdh.cpp 3686 2008-10-13 15:07:24Z Osswald2 $
...
void cSDH::SetAxisValueVector( std::vector<int> const& axes,
    ...
    throw (cSDHLibraryException*)
{
          ...
          ai++, vi++ )
    {
        ...
        if ( (used & all_axes_used) != all_axes_used )
            // No, so send individual value to firmware
            (comm_interface.*ll_set)( *ai, &v );
        else
            // yes, so store for sending later
            all_values[ *ai ] = v;
    }

    if ( (used & all_axes_used) == all_axes_used )
        // All axes are used, so send them alltogether, in order to communicate as few as possible
        (comm_interface.*ll_set)( All, &(all_values[0]) );
}
//----------------------------------------------------------------------
    ...
    comm_interface( _debug_level-1 )
{
    ...
    f_max_motor_current_v[ nb_all_axes - 1 ] = 0.0;  // no current for virtual axis


    f_min_angle_v.assign( nb_all_axes, -90.0 );
    f_min_angle_v[ 0 ] = 0.0;                        // correct axis 0
    f_min_angle_v[ nb_all_axes - 1 ] = 0.0;          // correct virtual axis

    f_max_angle_v.assign( nb_all_axes, 90.0 );
    f_max_angle_v[ nb_all_axes - 1 ] = 0.0;          // correct virtual axis


    f_min_velocity_v.assign( nb_all_axes, 0.0 );

    f_max_velocity_v.assign( nb_all_axes, 100.0 );
    f_max_velocity_v[ 0 ] = 28.0;                    // correct axis 0
    f_max_velocity_v[ nb_all_axes - 1 ] = 0.0;       // correct virtual axis

    f_min_acceleration_v.assign( nb_all_axes, 0.0 );

    f_max_acceleration_v.assign( nb_all_axes, 1000.0 );
    f_max_acceleration_v[ nb_all_axes - 1 ] = 0.0;   // correct virtual axis
    ...
    throw (cSDHLibraryException*)
{
          ...
          si++ )
    {
        ...
        if ( *si < NUMBER_OF_AXES )
            rv.push_back( temperatures_axes[ *si ] );
        else
            rv.push_back( temperatures_electronics[ *si - NUMBER_OF_AXES ] );
    }
    return rv;
}
//----------------------------------------------------------------------
    ...
    throw (cSDHLibraryException*)
{
    ...
    if ( iSensor < NUMBER_OF_AXES )
    {
        ...
        return temperatures[ iSensor ];
    }
    temperatures = comm_interface.temp_electronics();
    return temperatures[ iSensor-NUMBER_OF_AXES ];
}
//----------------------------------------------------------------------


void cSDH::OpenRS232(  int _port, unsigned long _baudrate, double _timeout )
    throw (cSDHLibraryException*)
{
    //---------------------
    ...
    com = new cRS232( _port, _baudrate, _timeout );
...
void cSDH::OpenCAN_ESD(  int _net,  unsigned long _baudrate, double _timeout, int32_t _id_read, int32_t _id_write )
    throw (cSDHLibraryException*)
{
#if WITH_ESD_CAN
    //---------------------
    ...
    cdbg << "cSDH.OpenCAN() successfully opened CAN port.\n";
#else
...
void cSDH::OpenCAN_ESD( NTCAN_HANDLE _ntcan_handle, double _timeout, int32_t _id_read, int32_t _id_write )
    throw (cSDHLibraryException*)
{
	std::cout << "OPEN ESD CAN" <<std::endl;
#if WITH_ESD_CAN
    //---------------------
    // Try to create a cSDHSerial object and save as member comm_interface:
	std::cout << "ESD CAN" <<std::endl;
    if ( _timeout < 0.0 )
        _timeout = 0.0;
    std::cout << "A" <<std::endl;
    if ( com )
    ...
    throw(cSDHLibraryException*)
{
    ...
    comm_interface.power( All, false );
    ...
    throw(cSDHLibraryException*)
{
    ...
    // As there is only one type of controller for now there is
    // nothing to do here
    if (controller != eCT_POSE )
    {
...
std::vector<double> cSDH::GetAxisMaxMotorCurrent( std::vector<int> const& axes )
    throw (cSDHLibraryException*)
{
         ...
         ai++, vi++ )
    {
        ...
        *vi = uc_motor_current->ToExternal( f_max_motor_current_v[ *ai ] );
    }
    ...
    throw (cSDHLibraryException*)
{
    ...
    do
    {
        ...
        for( si =  states.begin();
             ...
             si++ )
        {
            finished = finished  &&  (*si != eAS_POSITIONING );
        }
    ...
    throw (cSDHLibraryException*)
{
    ...
    return uc_angular_velocity->ToExternal( comm_interface.vlim( iAxis, NULL )[ iAxis ] );
}
//----------------------------------------------------------------------
