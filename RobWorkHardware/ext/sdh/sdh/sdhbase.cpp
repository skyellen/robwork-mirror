  ...
  \internal
    ...
    \subsection sdhlibrary_cpp_sdhbase_cpp_details SVN related, detailed file specific information:
      ...
      \par SVN file revision:
        $Id: sdhbase.cpp 3686 2008-10-13 15:07:24Z Osswald2 $
...
cSDHBase::cSDHBase( int debug_level ) :
    ...
    // init members:
    cdbg( (debug_level > 0), "magenta", g_sdh_debug_log ),
    NUMBER_OF_AXES( 7 ),
    NUMBER_OF_FINGERS( 3 ),
...
{
    if ( 0 < grasp_id  &&  grasp_id < eGID_DIMENSION )
...
{
    if ( 0 < controller_type  &&  controller_type < eCT_DIMENSION )
        return grasp_id_name[ controller_type ];
    else
