...
int main( int argc, char** argv )
{
    ...
    try
    {
        ...
        hand.GetNumberOfAxes();
        std::vector<double> maxCurr = hand.GetAxisMotorCurrent(axes);
        std::vector<double> maxCurrDiv = maxCurr;
        for(size_t i=0;i<maxCurr.size();i++){
        	maxCurrDiv[i] = maxCurr[i]/10;
        }
        ...
        for (int i=0; i<3; i++ )
        {
            ...
            hand.SetAxisMotorCurrent(axes, maxCurrDiv );
            ...
            hand.MoveFinger( iFinger, false);

            hand.SetAxisMotorCurrent(axes, maxCurr );
