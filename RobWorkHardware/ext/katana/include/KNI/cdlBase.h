...
/*! \brief Last Operation Values
 *
 * These return values are used by the lastOP() operations; 'lopDONE' means
 * that the last operation has been successful and a 'lopFAIL' return value
 * means that the last operation has failed.
 */

enum TLastOP {
	lopDONE,	//!< last operation has been successful
	lopFAIL		//!< last operation has failed
};
...
protected:
	...
	TLastOP	lop; //!< Stores the value of the last operation.

public:
	/*! \brief Returns the status of the last operation
	 *
	 * This function returns the success status of the last operation. If the
	 * last operation has been successful it return 'lopDONE', otherwise it
	 * returns 'lopFAIL'.
	 */
	TLastOP lastOP() { return lop; }
