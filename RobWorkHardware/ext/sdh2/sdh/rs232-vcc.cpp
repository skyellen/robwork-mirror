  ...
  \internal
    ...
    \subsection sdhlibrary_cpp_rs232_vcc_cpp_details SVN related, detailed file specific information:
      ...
      \par SVN file revision:
        $Id: rs232-vcc.cpp 4766 2009-08-31 13:46:47Z Osswald2 $
 ...
 */
#define SDH_RS232_VCC_DEBUG_ASCII 0
...
{
  DBG( dbg << "cRS232::SetTimeout( " << _timeout << ")\n"; )
  if ( _hCOM != INVALID_HANDLE_VALUE )
  {
    ...
    throw (cRS232Exception*)
{
    ...
    memset(datap, 0, size*sizeof(unsigned char));
    DWORD offset=0;
    DWORD bytes_read;
    BOOL rc;
    do
    {
        ...
        rc = ReadFile(_hCOM, &datap[offset], 1, &bytes_read, &o);
#else
        rc = ReadFile(_hCOM, &datap[offset], 1, &bytes_read, NULL ); //async , &o);
#endif
        if ( rc == 0 )
        {
#if SDH_RS232_VCC_ASYNC
            ...
            if ( last_error == ERROR_IO_PENDING )
            {
                ...
                case WAIT_OBJECT_0:             // ReadFile event
                    break;
                case WAIT_TIMEOUT:
                    DBG( dbg << "cRS232::Read: WAIT_TIMEOUT bytes_read = " << bytes_read << "\n"; )
        ...
        {
            dbg << ((unsigned int) (datap[offset+i]));
#if SDH_RS232_VCC_DEBUG_ASCII
            dbg << "='" << (char) (datap[offset+i]) << "' ";
#else
            dbg << " ";
#endif
        }
        ...
        if ( bytes_read == 0 )
        {
             ...
             DBG( dbg << "cRS232::Read timeout! bytes_read = 0\n"; )
#if SDH_RS232_VCC_ASYNC
             CloseHandle(o.hEvent);
#endif
             throw new cRS232Exception( cMsg( "Timeout while reading data from RS232 port %d = COM%d", port, port+1 ) );
...
#if SDH_RS232_VCC_DEBUG
    dbg << "cRS232::write wrote " << len << "/" << dwWritten << " bytes: ";
    int i;
    for ( i = 0; i < len; i++ )
    {
