     ...
     \internal
       ...
       \subsection sdhlibrary_cpp_sdh_demo_mimic_details SVN related, detailed file specific information:
         ...
         \par SVN file revision:
           $Id: demo-mimic.cpp 5000 2009-12-01 10:41:18Z Osswald2 $
...
cSDH* GetHand( cSDHOptions& options, cDBG& cdbg, int nb )
{
    ...
    {
        cdbg << "  Opening connection via ESD CAN, net " << options.net << " id_read/write " << options.id_read << "/" << options.id_write << "\n";
        hand->OpenCAN_ESD( options.net,
                          options.can_baudrate,
                          options.timeout,
                          options.id_read,
                          options.id_write );
    }
    ...
    {
        cdbg << "  Opening connection via PEAK CAN id_read/write " << options.id_read << "/" << options.id_write << "\n";
    ...
    {
        cdbg << "  Opening connection via RS232 device "  << options.sdh_rs_device << " port " << options.sdhport << "\n";
