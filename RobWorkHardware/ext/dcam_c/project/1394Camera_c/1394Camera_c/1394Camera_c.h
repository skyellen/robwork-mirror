...
extern "C" {
	...
	CONTROLHANDLE CAMAPIC GetCameraControl_c(CameraID camID, CAMERA_FEATURE fID);
  ...
  // Optional Functions
	...
	///// *****************************************
	//     NOW COMES THE CAMERA CONTROL FUNCTIONS

	//CONTROLHANDLE createCameraControlInstance(CameraID camID, CAMERA_FEATURE feature);
	//void releaseCameraControlInstance(CONTROLHANDLE ctrlID);

	// Inquiry Stuff
	int CAMAPIC Inquire_c(CONTROLHANDLE ctrlID,unsigned long *pRawData = NULL);
	bool CAMAPIC  HasPresence_c(CONTROLHANDLE ctrlID);         ///< Accessor for m_InquiryReg.present    \see Inquire()
	bool CAMAPIC HasAbsControl_c(CONTROLHANDLE ctrlID);       ///< Accessor for m_InquiryReg.absctl     \see Inquire()
	bool CAMAPIC HasOnePush_c(CONTROLHANDLE ctrlID);          ///< Accessor for m_InquiryReg.onepush    \see Inquire()
	bool CAMAPIC HasReadout_c(CONTROLHANDLE ctrlID);          ///< Accessor for m_InquiryReg.readout    \see Inquire()
	bool CAMAPIC HasOnOff_c(CONTROLHANDLE ctrlID);            ///< Accessor for m_InquiryReg.onoff      \see Inquire()
	bool CAMAPIC HasAutoMode_c(CONTROLHANDLE ctrlID);         ///< Accessor for m_InquiryReg.automode   \see Inquire()
	bool CAMAPIC HasManualMode_c(CONTROLHANDLE ctrlID);       ///< Accessor for m_InquiryReg.manualmode \see Inquire()
	void CAMAPIC GetRange_c(CONTROLHANDLE ctrlID,unsigned short *min, unsigned short *max);
	void CAMAPIC GetRangeAbsolute_c(CONTROLHANDLE ctrlID,float *fmin, float *fmax);
	
	// Status Stuff
	int CAMAPIC Status_c(CONTROLHANDLE ctrlID,unsigned long *pRawData = NULL);
	// Status Accessors
	bool CAMAPIC StatusPresence_c(CONTROLHANDLE ctrlID);      ///< Accessor for m_StatusReg.present     \see Status()
	bool CAMAPIC StatusAbsControl_c(CONTROLHANDLE ctrlID);    ///< Accessor for m_StatusReg.abscontrol  \see Status()
	bool CAMAPIC StatusOnOff_c(CONTROLHANDLE ctrlID);         ///< Accessor for m_StatusReg.onoff       \see Status()
	bool CAMAPIC StatusOnePush_c(CONTROLHANDLE ctrlID);      ///< Accessor for m_StatusReg.automode    \see Status()
	bool CAMAPIC StatusAutoMode_c(CONTROLHANDLE ctrlID);      ///< Accessor for m_StatusReg.automode    \see Status()
	void CAMAPIC GetValue_c(CONTROLHANDLE ctrlID,unsigned short *v_lo, unsigned short *v_hi=NULL);
	void CAMAPIC GetValueAbsolute_c(CONTROLHANDLE ctrlID,float *f);
	// Status Mutators
	int CAMAPIC SetAbsControl_c(CONTROLHANDLE ctrlID,BOOL on); ///< Mutator for m_StatusReg.AbsControl \return Same as SetStatus()
	int CAMAPIC SetOnOff_c(CONTROLHANDLE ctrlID,BOOL on);      ///< Mutator for m_StatusReg.onoff      \return Same as SetStatus()
	int CAMAPIC SetOnePush_c(CONTROLHANDLE ctrlID,BOOL on);    ///< Mutator for m_StatusReg.onepush    \return Same as SetStatus()
	int CAMAPIC SetAutoMode_c(CONTROLHANDLE ctrlID,BOOL on);   ///< Mutator for m_StatusReg.automode   \return Same as SetStatus()
	int CAMAPIC SetValue_c(CONTROLHANDLE ctrlID,unsigned short v_lo, unsigned short v_hi=0);
	int CAMAPIC SetValueAbsolute_c(CONTROLHANDLE ctrlID,float f); 
	
	// Identification Stuff
	CAMAPIC const char *GetName_c(CONTROLHANDLE ctrlID);
	CAMAPIC const char *GetUnits_c(CONTROLHANDLE ctrlID);
	CAMERA_FEATURE CAMAPIC GetFeatureID_c(CONTROLHANDLE ctrlID);
