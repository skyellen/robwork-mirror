...
std::vector<Frame*> ParallelLegHEXAPOD(FixedFrame *base, boost::shared_ptr<Tree> &tree, Transform3D<double> &legBase, Transform3D<double> &tool, double yaw, double pitch){
    ...
    serialChain.push_back(RevoluteJoint::make(serialChain.back(),"joint1",Transform3D<>::craigDH( 0, 0, 0, yaw)));
    serialChain.push_back(RevoluteJoint::make(serialChain.back(),"joint2",Transform3D<>::craigDH( Pi/2.0, 0, 0, Pi/2.0+pitch)));
    serialChain.push_back(new PrismaticJoint(serialChain.back(),"joint3",Transform3D<>::craigDH( Pi/2.0, 0, 0, 0)));
    serialChain.push_back(RevoluteJoint::make(serialChain.back(),"joint4",Transform3D<>::craigDH( -Pi/2.0, 0, 0, Pi/2.0)));
    serialChain.push_back(RevoluteJoint::make(serialChain.back(),"joint5",Transform3D<>::craigDH( Pi/2.0, 0, 0, -Pi/2.0)));
    serialChain.push_back(RevoluteJoint::make(serialChain.back(),"joint6",Transform3D<>::craigDH( Pi/2.0, 0, 0, Pi/2.0)));
...
BOOST_AUTO_TEST_CASE(ParallelDeviceTest)
{
    ...
    { // test of a HexaPod from microbotics
        ...
        Transform3D<> baseTA(
            Vector3D<>(-60.206416539249155,
                       -11.580306882912305,
                       0),
            Rotation3D<>(0,0,1,0,-1,0,1,0,0));

        Transform3D<> toolTA(
            Vector3D<>(14.533675867691082, -8.586935664729255, 5.486283046170201),
            Math::zyxToRotation3D<double>(0,-0.5058297976189102, 0.2554802362083838));

        std::vector<Frame*> legA =
            ParallelLegHEXAPOD(
                world,tree,baseTA,toolTA,-0.2554802362083838, 0.5058297976189102);

        /*Leg B*/
        Transform3D<> baseTB(
            Vector3D<>(
                -40.132048213846446, -46.350132752361176,0),
            Rotation3D<>(0,0,1,0,-1,0,1,0,0));

        Transform3D<> toolTB(
            Vector3D<>(-2.6407792002537804, 16.673016630261774, 5.486283046154413),
            Math::zyxToRotation3D<double>(0,-0.4486102166464917, 0.3501394675976276));

        std::vector<Frame*> legB =
            ParallelLegHEXAPOD(
                world,
                tree,
                baseTB,
