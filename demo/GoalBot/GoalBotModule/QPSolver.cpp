...
vector<double> QPSolver::InequalitySolve(const matrix<double>& G, const vector<double>& d, const matrix<double>& A, const vector<double>& b) {
    ...
    vector<double> x = zero_vector<double>(n);
    for (size_t i = 0; i<n; i++) { //Hack to initialize x with a valid value
	...
	if (bcompare(i) < b(i))
	    std::cout<<"Warning: Invalid Start Guess"<<std::endl;
	...
	matrix<double> Minv = PseudoInverse(M, 1e-12);
	...
	if (norm_inf(p_k) < QP_EPSILON) {
	    ...
	    if (min(lambda)>=0) {//-QP_EPSILON) { 
		vector<double> bcompare = prod(A, x);
		/*for (size_t i = 0; i<b.size(); i++)
		    if (bcompare(i) < b(i)) {
			std::cout<<"Warning: Return an Invalid result"<<i<<std::endl;
			std::cout<<"Warning: limit = "<<b(i)<<std::endl;
			std::cout<<"Warning: value = "<<bcompare(i)<<std::endl;
			std::cout<<"Warning: Error = "<<b(i)-bcompare(i)<<std::endl;
			std::cout<<"Warning: lambda = "<<lambda<<std::endl;
			std::cout<<"Warning: b = "<<b<<std::endl;
			for (IntList::iterator it = Wk.begin(); it != Wk.end(); ++it) 
			    std::cout<<"Warning: Constraint index "<<*it<<std::endl;
			char ch[3]; std::cin.getline(ch, 1);
		    }*/
    ...
    std::cout<<"!!!Warning: Did not terminate properly - Sets result between limits "<<Wk.size()<<std::endl;
