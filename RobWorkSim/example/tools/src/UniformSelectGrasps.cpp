...
int main(int argc, char** argv)
{
    ...
    if( argc < 3 ){
	    ...
	    std::cout << "- Arg 1 name of grasp tasks file" << std::endl;
	    std::cout << "- Arg 2 name of output grasp task file" << std::endl;
	    std::cout << "- Arg 3 filtering angle in Degree\n" << std::endl;
	    return 0;
	}

	std::string grasptask_file(argv[1]);
	std::string grasptask_file_out( argv[2] );
	int angleThres = 25*Deg2Rad;
	if(argc>2)
	    angleThres = atoi(argv[3])*Deg2Rad ;
	...
	BOOST_FOREACH(GraspSubTask& stask, gtask_exp->getSubTasks()){
	    BOOST_FOREACH(GraspTarget& target,stask.targets ){
            ...
            result.clear();
            GraspResult::Ptr gres = target.result; //node.valueAs<GraspResult::Ptr>();

            nntree->nnSearchRect(key-diff,key+diff, result);
            size_t nrNeighbors = result.size();
            if(gres->qualityAfterLifting.size()>1) {
                gres->qualityAfterLifting[1] = nrNeighbors*(1.0/maxNeigh);
	        } else {
                gres->qualityAfterLifting = Q(2, 0.0, nrNeighbors*(1.0/maxNeigh) );
            }
