...
namespace {
	...
	ublas::matrix<double> getBlockDiagInertia(Device &dev, std::vector<Link*>& parents,
											  Link* current){
...
void Link::rollBack(State &state){
    _torque = _torqueRB;
    _force = _forceRB;
    ...
    _vel = _velRB;
    std::cout << "Link: ROLLBACK " << _vel << " " << _pos << std::endl;
    //std::cout << _wTb << std::endl;
    //std::cout << _baseTb << std::endl;
    //std::cout << _jac << std::endl;
}

void Link::saveState(double h, rw::kinematics::State& state){
    ...
    _velRB = _vel;
    std::cout << "Link: SAVESTATE " << _vel << " " << _pos << std::endl;
    //std::cout << _wTb << std::endl;
    //std::cout << _baseTb << std::endl;
    //std::cout << _jac << std::endl;
}

void Link::resetState(rw::kinematics::State &state){
	_vel = 0;
	_pos = *_jointFrame.getQ(state);
	
	//std::cout << "Link: Update position! " << _pos << " " << _vel << " " << h << std::endl;
...
rw::math::Vector3D<> Link::getPointVelW(const rw::math::Vector3D<>& wPp){
...
void Link::updatePosition(double h, State &state){
...
rw::math::InertiaMatrix<> Link::getEffectiveMassW(const rw::math::Vector3D<>& wPc){
	using namespace boost::numeric::ublas;
...
rw::math::InertiaMatrix<> Link::getEffectiveMass(){
	using namespace boost::numeric::ublas;
	std::cout << "Eff mass";
	// calculate the block diagonal inertia matrix of all child bodies
	// the matrix has n blocks where n is number of dof
...
void Link::updateImpulse(){
	std::cout << "Link: Update impulse!" << std::endl;
...
void Link::updateVelocity(double h, State &state){
	std::cout << "Link: Update velocity!" << _jointFrame.getName() << std::endl;
...
void Link::addForceWToPosW(const rw::math::Vector3D<>& force,
...
void Link::addImpulseWToPosW(const rw::math::Vector3D<>& impulse,
                       const rw::math::Vector3D<>& pos){
	
	//std::cout << "Link: Impulse at: " << pos << std::endl;
	// transform the force into body frame description
