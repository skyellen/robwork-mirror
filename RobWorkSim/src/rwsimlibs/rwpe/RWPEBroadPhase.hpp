...
#ifndef RWSIMLIBS_RWPE_RWPEBROADPHASE_HPP_
#define RWSIMLIBS_RWPE_RWPEBROADPHASE_HPP_

/**
 * @file RWPEBroadPhase.hpp
 *
 * \copydoc rwsimlibs::rwpe::RWPEBroadPhase
 */
 ...
 *
 * This is useful for RWPEWorld and RWPEIsland engines where bodies are added and removed
 * dynamically.
 *
...
public:
	 ...
	 */
	RWPEBroadPhase(rw::common::Ptr<const rwsim::dynamics::DynamicWorkCell> dwc);

	//! @brief Destructor.
	virtual ~RWPEBroadPhase();
	 ...
	 * @param state [in] the state to check.
	 * @param log [in] pointer to logging util (optional).
	 * @return true if there are penetrations, false otherwise.
	 */
	bool maxPenetrationExceeded(rw::common::Ptr<const rwsim::contacts::ContactDetector> detector, const rw::kinematics::State& state, class RWPELogUtil* log = NULL);
...
#endif /* RWSIMLIBS_RWPE_RWPEBROADPHASE_HPP_ */
