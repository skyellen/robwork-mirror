...
RWPEBroadPhase::RWPEBroadPhase(rw::common::Ptr<const DynamicWorkCell> dwc):
	_dwc(dwc),
...
void RWPEBroadPhase::addObject(Object::Ptr object, bool dynamic) {
...
void RWPEBroadPhase::removeObject(Object::Ptr object) {
    ...
    BOOST_FOREACH(Geometry::Ptr geom, geoms){
        ...
        if (model->getGeometryIDs().size() == 1) {
            if (_frames.erase(geoframe)!=1)
            	RW_THROW("RWPEContactDetector (removeObject): did not erase frame \"" << geoframe->getName() << "\" exactly once in set of frames.");
...
RWPEBroadPhase::FramePairList RWPEBroadPhase::broadPhase(const State& state) {
...
bool RWPEBroadPhase::maxPenetrationExceeded(rw::common::Ptr<const ContactDetector> detector, const State& state, RWPELogUtil* log) {
	bool collision = false;
	std::vector<CollisionStrategy::Result> logCheck;
	...
	BOOST_FOREACH(const FramePairConst& pair, frames) {
		...
		for (itA = geoToPModelA.begin(); itA != geoToPModelA.end() && !collision; itA++) {
			for (itB = geoToPModelB.begin(); itB != geoToPModelB.end() && !collision; itB++) {
				...
				collision = _collisionStrategy->inCollision(modelA,wTa,modelB,wTb,data);
				if (log != NULL) {
					logCheck.push_back(data.getCollisionData());
				}
			}
		}
		if (collision)
	...
	if (log != NULL) {
		log->addCollisionResults("Collision Check",logCheck,RWPE_LOCATION);
	}
	return collision;
}

ProximityFilterStrategy* RWPEBroadPhase::getProximityFilterStrategy() const {
	return _bpStrategy;
}

ProximityFilterStrategy* RWPEBroadPhase::getEmptyBPStrategy(rw::common::Ptr<const DynamicWorkCell> dwc) {
	ProximitySetup rules;
	rules.setUseIncludeAll(false);
	return new BasicFilterStrategy(dwc->getWorkcell(),rules);
}

const ProximitySetup* RWPEBroadPhase::getDefaultProximitySetup(rw::common::Ptr<const DynamicWorkCell> dwc) {
