...
class RWPERollbackMethodRidder::RollbackDataRidder: public RollbackData {
public:
...
RWPERollbackMethodRidder::RWPERollbackMethodRidder() {
}

RWPERollbackMethodRidder::~RWPERollbackMethodRidder() {
}

RWPERollbackMethod::RollbackData* RWPERollbackMethodRidder::createData() const {
	return new RollbackDataRidder();
}

double RWPERollbackMethodRidder::getTimestep(SampleSet& samples, RollbackData* data) const {
	fixSampleSet(samples);
	RollbackDataRidder* const ridderData = dynamic_cast<RollbackDataRidder*>(data);
	if (ridderData == NULL)
		RW_THROW("RWPERollbackMethodRidder (getTimestep): the rollback data given to the Ridder method was not of the type RollbackDataRidder!");
	if (samples.size() < 2)
		RW_THROW("RWPERollbackMethodRidder (getTimestep): the size of the SampleSet should be at least two!");
		...
		if (sample1.distance.has(ridderData->curPair)) {
			...
			if (d1*d2 < 0 && d1*d4 < 0) {
				...
				RW_THROW("RWPERollbackMethodRidder (getTimestep): could not do false-position part.");
			}
			samples.clear();
		...
		RW_THROW("RWPERollbackMethodRidder (getTimestep): the size of the SampleSet should be maximum four!");
	}
	return 0;
}

double RWPERollbackMethodRidder::threeSamples(const Sample& sample1, const Sample& sample2, const Sample& sample3, RollbackDataRidder* data) {
	...
	if (t4 > t1 && t4 < t3)
		...
		RW_THROW("RWPERollbackMethodRidder (threeSamples): could not place t4 in correct region.");
	return t4;
}

void RWPERollbackMethodRidder::fixSampleSet(SampleSet& samples) {
...
std::pair<const Frame*, const Frame*> RWPERollbackMethodRidder::findDeepest(const Sample& sample) {
