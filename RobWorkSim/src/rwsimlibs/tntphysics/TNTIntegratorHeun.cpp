...
TNTIntegratorHeun::TNTIntegratorHeun():
	TNTIntegrator(NULL),
	_euler(new TNTIntegratorEuler())
{
}

TNTIntegratorHeun::TNTIntegratorHeun(const TNTRigidBody* body):
	TNTIntegrator(body),
	_euler(new TNTIntegratorEuler(body))
{
}
...
void TNTIntegratorHeun::integrate(const Wrench6D<> &netFT, double h, TNTRigidBody::RigidConfiguration &configuration) const {
	if (getBody() == NULL)
		RW_THROW("TNTIntegratorHeun (integrate): There is no body set for this integrator - please construct a new integrator for the specific body to use.");
	RW_THROW("Not working yet!");
}

void TNTIntegratorHeun::positionUpdate(const Wrench6D<> &netFT, double h, TNTRigidBody::RigidConfiguration &configuration) const {
	if (getBody() == NULL)
		RW_THROW("TNTIntegratorHeun (positionUpdate): There is no body set for this integrator - please construct a new integrator for the specific body to use.");
...
/*
void TNTIntegratorHeun::velocityPrediction(const Wrench6D<> &netFT, double h, TNTRigidBody::RigidConfiguration &configuration) const {
	if (getBody() == NULL)
		RW_THROW("TNTIntegratorHeun (velocityPrediction): There is no body set for this integrator - please construct a new integrator for the specific body to use.");
	// Extract required info
	const rw::common::Ptr<const RigidBody> rwbody = getBody()->getRigidBody();
	const Transform3D<>& wTb = configuration.getWorldTcom();
	const Vector3D<>& R = wTb.P();
...
void TNTIntegratorHeun::velocityUpdate(const Wrench6D<> &netFTcur, const Wrench6D<> &netFTnext, double h, const TNTRigidBody::RigidConfiguration &configuration0, TNTRigidBody::RigidConfiguration &configurationH) const {
	if (getBody() == NULL)
		RW_THROW("TNTIntegratorHeun (velocityUpdate): There is no body set for this integrator - please construct a new integrator for the specific body to use.");
	// Extract required info
	const rw::common::Ptr<const RigidBody> rwbody = getBody()->getRigidBody();
	const double massInv = rwbody->getMassInv();
...
Eigen::Matrix<double, 6, 1> TNTIntegratorHeun::eqPointVelIndependent(
	const Vector3D<> point, double h,
	const TNTRigidBody::RigidConfiguration &configuration0,
	const TNTRigidBody::RigidConfiguration &configurationH,
	const Vector3D<>& Ftot0, const Vector3D<>& Ntot0,
	const Vector3D<>& FextH, const Vector3D<>& NextH) const
{
	if (getBody() == NULL)
		RW_THROW("TNTIntegratorHeun (eqPointVelIndependent): There is no body set for this integrator - please construct a new integrator for the specific body to use.");
	const rw::common::Ptr<const RigidBody> rwbody = getBody()->getRigidBody();
...
Eigen::Matrix<double, 6, 6> TNTIntegratorHeun::eqPointVelConstraintWrenchFactor(const Vector3D<> point, double h, const Vector3D<> constraintPos, const TNTRigidBody::RigidConfiguration &configuration, const TNTRigidBody::RigidConfiguration &configurationGuess) const {
	if (getBody() == NULL)
		RW_THROW("TNTIntegratorHeun (eqPointVelConstraintWrenchFactor): There is no body set for this integrator - please construct a new integrator for the specific body to use.");
	const rw::common::Ptr<const RigidBody> rwbody = getBody()->getRigidBody();
	const Transform3D<>& wTb = configuration.getWorldTcom();
	const Vector3D<>& R = wTb.P();
...
bool TNTIntegratorHeun::eqIsApproximation() const {
	return false;
}

const TNTIntegrator* TNTIntegratorHeun::getDiscontinuityIntegrator() const {
	return _euler;
}
