...
struct TNTRWConstraint::ComplianceDecomposition {
...
TNTRWConstraint::TNTRWConstraint(rw::common::Ptr<const Constraint> constraint, const TNTBody* parent, const TNTBody* child):
...
TNTRWConstraint::~TNTRWConstraint() {
	if (_spring != NULL)
		delete _spring;
	_spring = NULL;
}
...
void TNTRWConstraint::update(TNTIslandState &tntstate, const State& rwstate) {
	...
	if (newCompliance) {
		...
		if (_spring->linComp > 0 && _spring->cDec.linFixedDirs.size() > 0) {
			...
			if (_spring->linComp == _spring->cDec.linFixedDirs.size()) {
				_spring->cDec.modes[0] = TNTConstraint::Velocity;
		...
		if (_spring->angComp > 0 && _spring->cDec.angFixedDirs.size() > 0) {
			...
			if (_spring->angComp == _spring->cDec.angFixedDirs.size()) {
				_spring->cDec.modes[3] = TNTConstraint::Velocity;
	...
	const Eigen::VectorXd XcurD = getXd(wTparent, wTconstraint, tntstate, rwstate);
	...
	applyWrench(tntstate,Wrench6D<>(cartForceW,cartTorqueW));
}

void TNTRWConstraint::reset(TNTIslandState &tntstate, const State &rwstate) {
...
std::size_t TNTRWConstraint::getDimVelocity() const {
	if (_spring != NULL)
...
void TNTRWConstraint::deleteSpring() {
	delete _spring;
	_spring = NULL;
	_posParent = _pTc.P();
	_posChild = _cTc.P();
...
Eigen::VectorXd TNTRWConstraint::getX(const Transform3D<> &wTchild, const Transform3D<> &wTconstraint, const State &state) const {
...
Eigen::VectorXd TNTRWConstraint::getXd(const Transform3D<> &wTparent, const Transform3D<> &wTconstraint, const TNTIslandState& tntstate, const State &rwstate) const {
	const VelocityScrew6D<> velP = getParent()->getVelocityW(rwstate,tntstate);
	const VelocityScrew6D<> velC = getChild()->getVelocityW(rwstate,tntstate);
...
std::pair<Vector3D<>, Vector3D<> > TNTRWConstraint::toCartesian(const Eigen::VectorXd &reduced) const {
...
std::vector<TNTConstraint::Mode> TNTRWConstraint::getModes(rw::common::Ptr<const Constraint> constraint) {
