...
void TNTContactResolverHeuristic::solve(const std::vector<TNTContact*>& persistentContacts, double h, const TNTMaterialMap& map, const State &rwstate, TNTIslandState &tntstate) const {
	if (_solver == NULL)
	...
	while (repeat) {
		if (iterations == TNT_CONTACTRESOLVER_MAX_ITERATIONS)
			RW_THROW("TNTContactResolverHeuristic (solve): maximum number of iterations (" << TNT_CONTACTRESOLVER_MAX_ITERATIONS << ") reached in contact resolution.");
