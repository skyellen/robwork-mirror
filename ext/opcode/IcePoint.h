...
// Removed since it introduced the nasty "Point T = *Matrix4x4.GetTrans();" bug.......
		...
		inline_					Point(const float f[3]) : x(f[_X_]), y(f[_Y]), z(f[_Z])		{}
		//! Copy constructor
		...
		inline_	Point&			Set(const float f[3])					{ x  = f[_X_];	y  = f[_Y];	z  = f[_Z];			return *this;	}
		...
		//! Adds a vector
		inline_	Point&			Add(const float f[3])					{ x += f[_X_];	y += f[_Y];	z += f[_Z];			return *this;	}
		//! Adds vectors
		...
		//! Subtracts a vector
		inline_	Point&			Sub(const float f[3])					{ x -= f[_X_];	y -= f[_Y];	z -= f[_Z];			return *this;	}
		//! Subtracts vectors
