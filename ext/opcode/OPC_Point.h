...
#include <math.h>
	...
	class ICEMATHS_API Point
	{
		...
		inline_					Point(float f[3]) : x(f[_X_]), y(f[_Y]), z(f[_Z])			{}
		//! Copy constructor
		...
		inline_	Point&			Set(float f[3])							{ x  = f[_X_];	y  = f[_Y];	z  = f[_Z];			return *this;	}
		...
		//! Adds a vector
		inline_	Point&			Add(float f[3])							{ x += f[_X_];	y += f[_Y];	z += f[_Z];			return *this;	}
		//! Adds vectors
		...
		//! Subtracts a vector
		inline_	Point&			Sub(float f[3])							{ x -= f[_X_];	y -= f[_Y];	z -= f[_Z];			return *this;	}
		//! Subtracts vectors
