...
#define __ICEHPOINT_H__
	...
	class ICEMATHS_API HPoint : public Point
	{
		...
		inline_	HPoint&		Set(const float f[4])							{ x  = f[_X_];	y  = f[_Y];	z  = f[_Z];	w  = f[_W];		return *this;	}
		...
		//! Add a vector
		inline_	HPoint&		Add(const float f[4])							{ x += f[_X_];	y += f[_Y];	z += f[_Z];	w += f[_W];		return *this;	}

		//! Subtract a vector
		inline_	HPoint&		Sub(float _x, float _y, float _z, float _w )	{ x -= _x;		y -= _y;	z -= _z;	w -= _w;		return *this;	}
		//! Subtract a vector
		inline_	HPoint&		Sub(const float f[4])							{ x -= f[_X_];	y -= f[_Y];	z -= f[_Z];	w -= f[_W];		return *this;	}
